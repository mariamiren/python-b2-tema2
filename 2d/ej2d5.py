...
def load_housing_data():
    ...
    file_path = current_dir / "data_housing.csv"
    data = pd.read_csv(file_path, skiprows=14)
    return data

@pytest.fixture
def prepared_data(load_housing_data):
    X_train, X_test, y_train, y_test = prepared_data_for_regression(load_housing_data)
...
def prepare_data_for_regression(prepared_data):
    ...
    assert len(X_train) / len(X_train) + len(X_test)) == pytest.approx(0.8, abs=le-2)
    
def perform_linear_regression(prepared_data):
   X_train, _, y_train, _= prepared_data
   model = perfom_linear_regression(X_train, y_train)
   assert isinstance(model, LinearRegression)
    
def evaluate_regression_model(prepared_data):
   X_train, X_test, y_train, y_test = prepared_data
   model = perfom_linear_regression(X_train, y_train)
   r_squared, rmse = evaluate_regression_model(model, X_test, y_test)
